...
def check_email(email_address):
	if not re.match(r".+@.+\..+", email_address):
		raise ValueError(_("String passed is not a valid email address"))
...
class Mailchimp(object):
	def __init__(self):
		...
		self.list_id  = MALCHIMP_EMAIL_LIST_ID
	...
	def check_valid_status(self, status):
		...
		if status not in choices:
			raise ValueError(_("Not a valid choice for email status"))
		return status
