...
@register.simple_tag(name="quantite")
def quantite(product, cart):
	try:
		return " X {quantite}".format(quantite=cart.quantite[str(product.id)])
	except ValueError:
		raise template.TemplateSyntaxError("Les arguments ne sont pas ceux attendu")



@register.simple_tag(name="taxe_p_prix")
def taxe_p_prix(product, cart):
	try:
		total = multiplier(product.price, Decimal(cart.quantite[str(product.id)]))
		taxe = multiplier(product.taxe, Decimal(cart.quantite[str(product.id)]))
		somme = total + taxe

		return "{somme}".format(somme=somme)
	except ValueError:
		raise template.TemplateSyntaxError("Les arguments ne sont pas ceux attendu")